    ...
    async scrape(): Promise<ClimbingLocation[]> {
        ...
        try {
            ...
            while (pageNum <= totalPages) {
                ...
                await this.page.waitForSelector('.table.route-table', {timeout: 30000})
                ...
                const boulderRoutesOnPage = await this.page.evaluate(() => {
                    ...
                    return rows.map((row) => {
                      ...
                      const routeLink = nameElement ? (nameElement as HTMLAnchorElement).href : "";
                  ...
                  for (const boulder of boulderRoutesOnPage) {
                    ...
                    const coordinates = await this.page.evaluate((): {lat: number, lng: number} | null => {
                        const gpsRow = Array.from(document.querySelectorAll('.description-details tr'))
                            .find(row => row.querySelector('td')?.textContent?.includes('GPS:'))
                            ...
                            const match = coordinatesText.match(/(-?\d+\.\d+),\s*(-?\d+\.\d+)/);
